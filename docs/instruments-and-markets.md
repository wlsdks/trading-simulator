...
본 문서는 오너 확정 신규 스코프(레버리지 롱/인버스 1000배·ETF/ETN·옵션·선물·무기한·코인·3개 시장·소셜)를 기존 policies.md(12도메인·152정책)의 톤·번호체계·회계 불변식과 정합하게 **상품/시장/엔진 레이어**로 종합한다. 정책 상세 규칙·사이드이펙트·엣지케이스는 policies.md에 편입되며, 본 문서는 **계약명세·공식·엔진·MVP 배선**을 단일 진실로 제공한다.
...
- 청산 이벤트는 프로필 리스크지표에 '**청산 횟수·최대 레버리지**'로 노출(공개 프로필 숨김 불가 — 리스크 투명성). NLV가 0 근처·마이너스 근접해도 순위는 손실 그대로 반영(GRF-01 Rz<0 선형)
