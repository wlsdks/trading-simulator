...
## ⚠️ 보강 필요 (Round 후속)
이 문서의 과거 미확정 항목 중 **상태관리와 차트 방향은 dev-spec.md §1 GAP #1~#2에서 해소**되었다: 앱 상태는 Zustand, 서버 상태는 TanStack Query, 차트는 custom Skia surface를 기준으로 한다. 남은 보강 항목:
- **상위 차트 구현 범위:** Skia 직접 구현을 기준으로 하되, 캔들/라인/볼륨/마커 이상의 고수준 차트 킷 채택은 SDK 57/New Arch 호환 확인 후 별도 결정.
