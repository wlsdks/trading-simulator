...
**한줄요약:** 게스트 온보딩·핵심 트레이딩·포트폴리오 회계부터 파생/마진 핵심 화면, 학습·게임화·수집·푸시·소셜(프로필 열람·공개범위·팔로우·다차원 리더보드)·수익화·법무/개인정보·커뮤니티 트러스트&세이프티까지 유즈케이스를 표준 형식(액터·목표·사전조건·흐름·사후조건·관련정책)으로 정리하고, 액터 정의·유즈케이스 다이어그램·우선순위 표를 제공한다.
...
| **휴면 사용자** | 최근 14일 이상 핵심 게임 액션(거래·미션·수집·대회·앱 열람)이 없는 사용자. OS 푸시 거부자는 인앱 알림함으로만 윈백한다. | UC-B37 |
...
| **OS 푸시 서비스(APNs/FCM)** | 권한 프롬프트·푸시 전달·열람 딥링크를 담당한다. 조용시간·일일 상한 판정은 앱/서버가 선행한다. |
...
| UC-A15 | 파생·마진 종목 상세 화면 | A | 사용자 |
| UC-A16 | 레버리지 설정 플로우 | A | 사용자 |
| UC-A17 | 청산가·마진콜 경고 | A | 사용자 |
| UC-A18 | 부분청산 통지 | A | 시스템(앱)·사용자 |
| UC-A19 | 옵션 그릭스 표시 | A | 사용자 |
| UC-A20 | 펀딩레이트 카운트다운 | A | 사용자 |
| UC-A21 | 차트 고급 상호작용(deferred) | A | 사용자 |
| UC-A22 | 종목 상세 미시구조 뷰(deferred) | A | 사용자 |
| UC-A23 | 뉴스 읽기·히스토리(deferred) | A | 사용자 |
| UC-A24 | 마켓 펄스 렌즈 탐색(deferred) | A | 사용자 |
| UC-A25 | breaking-news landing(deferred) | A | 사용자 |
| UC-A26 | 이벤트 반응 플로우(deferred) | A | 사용자 |
| UC-A27 | 유상청약 결정(deferred) | A | 사용자 |
| UC-B01 | 최초 게스트 온보딩(30초 첫 매수 + 입성 / 플레이어 탄생 배지) | B | 신규 사용자(게스트) |
...
| UC-B36 | 도감/컬렉션 발견·세트 완성 | B | 사용자 |
| UC-B37 | 푸시 권한 프라이밍·알림함 fallback·윈백 | B | 복귀/휴면 사용자 |
...
- **주요 흐름:**
  ...
  7. '입성 / 플레이어 탄생' 배지 지급(거래가 아닌 정체성 진입을 축하) → 홈/마켓 착지.
- **대안 흐름:**
...
- **관련 정책:** 연령게이트(PIPA14/GDPR16/COPPA13, LEG-05), 온보딩 미세 면책 배너+소셜/결제/대회 진입 전 전면 동의 로그(LEG-02), 매매/P&L 축하 연출 금지 및 '입성 / 플레이어 탄생' 정체성 축하 허용(R4/LEG-07), 데이터 최소화·지연 가입(DP-01), schemaVersion 마이그레이션(ACC-09/DP-04), 튜토리얼 거래 격리(ACC-10).
...
### UC-A15 · 파생·마진 종목 상세 화면

- **액터:** 사용자
- **목표:** 레버리지·숏·옵션·선물·무기한 등 고위험 상품의 현재가, 마진 상태, 위험 고지, 관련 주문 진입점을 한 화면에서 이해한다.
- **사전조건:** 대상 가상상품이 유니버스에 존재; 사용자 연령·고위험 고지 게이트 상태 확인 가능.
- **주요 흐름:**
  1. 종목 상세 진입 → 상품 배지(현물/레버리지/옵션/선물/무기한), 시뮬 라벨, 고위험 라벨을 헤더에 표시.
  2. 계좌 상태 카드에 freeCash·reservedMargin·marginLevel·청산가 요약·펀딩/이자 예정액을 표시.
  3. 하단 주문 CTA는 레버리지 설정(UC-A16), 옵션 그릭스(UC-A19), 펀딩 카운트다운(UC-A20) 등 상품별 모듈로 분기.
  4. 사용자가 미성년/미동의/고위험 미승인 상태이면 주문 CTA 대신 고지 게이트 또는 읽기 전용 안내를 표시.
- **대안 흐름:** 미보유 상품은 포지션 카드 빈상태; CASH 계좌는 MARGIN 전환 안내만 노출; 장마감/거래정지는 주문 CTA 비활성; 고위험 리그 격리 대상이면 보상/리더보드 격리 라벨 표시.
- **예외 흐름:** 마진 필드 누락 시 읽기 전용 + 오류 리포트; 시세 미수신 시 마지막 스냅샷과 지연 배지; 권한 불일치 시 주문 차단.
- **사후조건:** 사용자는 고위험 상품의 상태와 다음 행동을 이해하며, 주문 진입 전 필수 고지가 충족된다.
- **관련 정책:** ACT-15, ACC-21/22/23, LEV-01~08, GRF-16, LEG-07.

### UC-A16 · 레버리지 설정 플로우

- **액터:** 사용자
- **목표:** 레버리지 배율·격리마진·주문수량을 설정하고, 예상 청산가와 최대 손실을 확인한 뒤 주문한다.
- **사전조건:** MARGIN 계좌; 고위험 고지 통과; 대상 상품 레버리지 허용.
- **주요 흐름:**
  1. 레버리지 슬라이더/스테퍼(1x~상품 cap)와 증거금 입력을 표시.
  2. 입력 변경마다 예상 노티널·수수료·마진이자·청산가·부분청산 임계를 즉시 재계산.
  3. 3x 초과 또는 파생 노출이면 2스텝 확인과 '가상 자산·모의·원금 소진 가능' 고지 표시.
  4. 주문 제출 후 order-accepted light haptic, 체결 시 fill success-impact haptic. 컨페티 없음.
- **대안 흐름:** 낮은 잔고는 수량 자동 축소 제안; 미성년/고위험 제한은 읽기 전용; cap 초과 입력은 cap으로 clamp하고 설명.
- **예외 흐름:** 청산가 계산 실패·mark 미수신·계좌유형 불일치 시 제출 차단; 동시 주문으로 marginLevel 변동 시 최신 상태로 재검증.
- **사후조건:** 주문은 최신 마진 상태와 고지 이력으로 제출되거나 안전하게 거부된다.
- **관련 정책:** LEV-01/02/03/08, ORD-17, ACC-21/22, LEG-07, docs/design-tokens.md state feedback.

### UC-A17 · 청산가·마진콜 경고

- **액터:** 사용자
- **목표:** 마진콜·청산 위험을 이해하고, CTA 없이 위험 완화 선택지를 확인한다.
- **사전조건:** 열린 마진/파생 포지션 존재.
- **주요 흐름:**
  1. marginLevel이 warning 임계 접근 시 포지션 카드·상단 배너·알림함에 경고 표시.
  2. 청산가까지 거리, 유지증거금, mark 기준, 부분청산 가능성을 숫자+색+아이콘으로 표시.
  3. 조용시간에는 OS 푸시를 억제하고 인앱 알림함에 적재한다. 필수 위험 고지는 다음 앱 오픈 시 상단에 노출한다.
  4. 착지 화면은 포지션/리스크 상세이며 매수/매도 시트 프리필 금지.
- **대안 흐름:** 이미 청산가를 넘은 경우 UC-A18로 전환; 포지션 없음은 빈상태; 사용자가 알림 거부 시 인앱 알림함 fallback.
- **예외 흐름:** mark 급변으로 경고와 청산이 같은 tick에 발생하면 청산 통지를 우선; 계산 불일치 시 서버 권위값 표시.
- **사후조건:** 위험 고지와 suppression 이벤트가 기록되고, 사용자는 조용시간·wellbeing 규칙을 침해받지 않는다.
- **관련 정책:** LEV-04/05, NWS-05/06, GRF-15, LEG-07, UC-B37.

### UC-A18 · 부분청산 통지

- **액터:** 시스템(앱)·사용자
- **목표:** 강제청산이 발생했을 때 손실 자극을 과장하지 않고 원인·수량·남은 위험을 투명하게 알린다.
- **사전조건:** 서버 권위 청산 엔진이 부분청산 또는 전량청산을 확정.
- **주요 흐름:**
  1. liquidation event 수신 → 인앱 알림함과 포지션 상세에 사유 로그 표시.
  2. 청산 수량, 청산가, 청산수수료, 남은 포지션·marginLevel을 표기.
  3. '복기 보기'를 기본 CTA로 두고 재진입/복수매매 CTA는 제공하지 않는다.
- **대안 흐름:** 앱 미실행·푸시 허용·조용시간 아님이면 1일 상한 내 위험 알림 발송; 조용시간이면 인앱 적재 및 다음 오픈 시 표시; 전량청산이면 파산/리셋 흐름(UC-B26)로 연결.
- **예외 흐름:** 중복 청산 이벤트는 idempotency로 1회 표시; 통신 실패 시 다음 동기화에서 소급 표시.
- **사후조건:** 청산 원장과 사용자 통지가 일치하며, 조롱·자랑·축하 연출은 발생하지 않는다.
- **관련 정책:** LEV-04/05/06, ACC-22/23, SOC-05, LEG-07, UC-B37.

### UC-A19 · 옵션 그릭스 표시

- **액터:** 사용자
- **목표:** 옵션 계약의 delta/gamma/theta/vega/IV와 만기·행사가를 읽고, 손익이 아니라 위험 노출을 이해한다.
- **사전조건:** 옵션 유니버스(IP2)와 Black-Scholes/IV 엔진 사용 가능.
- **주요 흐름:** 계약 선택 → 그릭스 표(등폭 숫자) → 만기/행사가/IV 시나리오 토글 → 주문 CTA 또는 워치리스트 저장.
- **대안 흐름:** 표본 부족 IV는 N/A; 만기 임박은 경고 라벨; 숏옵션은 고위험 게이트.
- **예외 흐름:** 프라이싱 실패·음수 IV·만기 지난 계약은 거래정지/읽기 전용.
- **사후조건:** 그릭스 조회 이벤트와 고위험 고지 상태가 기록된다.
- **관련 정책:** MKT-15, ORD-24, ACC-20, GRF-16, LEG-07.

### UC-A20 · 펀딩레이트 카운트다운

- **액터:** 사용자
- **목표:** 무기한 포지션의 다음 펀딩 시간·예상 지급/수취 방향·계좌 영향 버킷을 확인한다.
- **사전조건:** 무기한 상품 또는 포지션 존재.
- **주요 흐름:** 포지션 카드에 T-minus 카운트다운 → 예상 funding sign·rate·지난 funding history → 펀딩 직전 threshold-cross selection haptic.
- **대안 흐름:** 포지션 없음은 상품 설명만; 조용시간엔 OS 푸시 대신 인앱 알림함; funding rate 급변은 경고 라벨.
- **예외 흐름:** 서버 시간 불일치 시 서버 UTC 기준으로 보정; rate 미수신 시 N/A와 마지막 갱신 시각 표시.
- **사후조건:** 펀딩 이벤트와 원장 반영이 서버 UTC 기준으로 일치한다.
- **관련 정책:** MKT-16, ACC-18, CRY-04, GRF-04, NWS-05.

### UC-A21~A27 · Deferred / lower-priority fun-surface UCs

> 이 티어는 엔진/정책은 존재하지만 P1 핵심 화면보다 우선순위가 낮은 사용자 표면이다. 각 UC는 추후 상세화 시 상태·edge·empty·error variant를 UC-A15~A20 수준으로 확장한다.

| ID | 제목 | 현재 범위 | 필수 variant |
|---|---|---|---|
| UC-A21 | 차트 고급 상호작용 | intraday aggregation, crosshair, event markers, account-context price split | 데이터 없음, 장마감, 이벤트 중첩, 접근성 대체표 |
| UC-A22 | 종목 상세 미시구조 뷰 | 합성 호가·심도·체결강도·상대 활동도 | seed-only vs display overlay 라벨, 거래정지, 낮은 유동성 |
| UC-A23 | 뉴스 읽기·히스토리 | 속보 피드, 정정, 루머 상태 히스토리 | 정정/부인, 템플릿 누락, 조용시간 다이제스트 |
| UC-A24 | 마켓 펄스 렌즈 탐색 | 거래대금·RVOL·체결강세·인기 렌즈 | 빈 렌즈, 플리커 방지, 시뮬 라벨 |
| UC-A25 | breaking-news landing | 뉴스+차트+이벤트 phase 착지 | 매수 CTA 금지, 손실 보유 악재 지연, 정정 우선 |
| UC-A26 | 이벤트 반응 플로우 | 이벤트 3막, 여진 냉각, 비추격 미션 | 추격매수 마찰, 루머 DENY, 이벤트 후 복기 |
| UC-A27 | 유상청약 결정 | 신주인수권·청약·AUTO_SELL/LAPSE 선택 | 현금 부족, 권리 실권, TERP 설명, externalFlows 미오염 |

---

## 6. 그룹 B — 온보딩·학습·게임화·소셜·수익화 상세

### UC-B01 · 최초 게스트 온보딩(30초 첫 매수 + 입성 / 플레이어 탄생 배지)

- **액터:** 신규 사용자(게스트)
- **목표:** 계정 가입 없이 30초 내 실패 불가능한 첫 매수를 완료하고, 세션 종료 전 '입성 / 플레이어 탄생' 배지 1개를 보장 획득한다(day-one identity achievement).
...
- **주요 흐름:**
  ...
  7. 매수 체결 컨페티 없이 '입성 / 플레이어 탄생' 배지 획득 연출 + '30초 학습 카드 보기' 넛지.
  8. 홈 착지, 오늘의 미션 2개 노출.
- **대안 흐름:**
  - 2a(연령 미달): UC-B02 차단 흐름 이동, 온보딩 중단.
  - 3a(전면 동의 미완료): 순수 로컬 게스트 첫 매수는 잠그지 않는다. 온보딩에는 미세 배너만 적용하고, 전면 면책 동의는 소셜·결제·대회 진입 시점에 재노출·필수화한다(동의 전 해당 기능 진입 불가).
...
- **관련 정책:** R4 매매/P&L 축하 금지 및 정체성 축하 허용(LEG-07), 온보딩 미세 면책 배너+소셜/결제/대회 진입 전 전면 고지(LEG-02), 연령 게이트(LEG-05), day-one identity achievement 보장, 튜토리얼 거래 별도 태그(ACC-10).
...
- **액터:** 사용자(모든 사용자)
- **목표:** 학습·리스크관리·탐험·도감·여정 카테고리 배지를 좋은 행동으로 획득하고 프로필 진열대에서 관리.
...
- **대안 흐름:**
  ...
  - 5a(축하 제한): 컨페티는 P&L 독립 배지(손절·분산·복기·도감 세트·좋은 판단 스트릭)에만 허용.
- **사후조건:** 배지·희귀도 기록; 프로필 진열대 갱신; 공유 카드 소스로 사용 가능.
- **관련 정책:** 도박 보상 배지 원천 배제(GRF-01/LEG-07), 축하 연출은 P&L 독립 성취에만(LEG-07), 도감/세트 보상(UC-B36), 공유 자랑은 배지·진도(수익률 아님/GRF-08/LEG-06).
...
- **액터:** 사용자(모든 사용자)
- **목표:** 판단 품질·수집·시나리오·리스크관리 XP를 누적해 레벨(입문~숙련)을 올리고 리셋되지 않는 영속 정체성을 형성.
- **사전조건:** XP 원장 초기화; 레벨 곡선(누적XP = 50×Lv²) 정의.
- **주요 흐름:**
  1. 시나리오 완주·신규 가상회사 첫 거래·손절 준수·위험조정 마일스톤·좋은 판단 스트릭·도감 세트 완성·positive-EV skill play로 XP 획득(거래량 XP=0).
  2. 누적 XP가 레벨 임계 초과 시 레벨업.
  3. 레벨업 알림 + 프로필·소셜 카드 레벨 표기 갱신.
  4. Lv10까지 빠르게, Lv20+부터 완만(롱테일).
- **대안 흐름:**
  - 1a(거래량 무관): 매매 횟수·거래대금·단순 수익 실현은 XP 미영향(과잉거래·도박화 차단).
  - 1c(XP 티어): 위험조정 마일스톤 +25/+75/+150 XP, 시나리오 완주 +60/+120 XP, 신규 가상회사 첫 거래 +10 XP(회사당 1회), 손절 준수 +20 XP, 좋은 판단 스트릭 3/7/14일 +40/+100/+220 XP, positive-EV skill play +80 XP, 도감 섹터/국가 세트 +150/+300 XP.
...
- **관련 정책:** 거래량 XP=0·P&L 직접 XP 금지(LEG-07), 판단 품질/도감/실력지표 XP(정본 policies.md GRF·game-modes §2.4), 레벨은 영속 정체성(ACC-02 리셋 정책), 소셜 카드 수익률 배제(GRF-08/LEG-06).
...
### UC-B36 · 도감/컬렉션 발견·세트 완성

- **액터:** 사용자
- **목표:** 가상회사를 발견·첫 거래해 도감 항목을 열고, 섹터/국가/시즌 세트를 완성해 XP·배지·코스메틱을 획득한다.
- **사전조건:** 가상회사 유니버스와 collection catalog 로드; 첫 거래 여부를 companyId 단위로 판정 가능.
- **주요 흐름:**
  1. 사용자가 신규 가상회사 상세를 열면 도감 미발견 상태(실루엣·섹터 힌트)를 표시.
  2. 해당 회사 첫 거래가 체결되면 도감 entry unlock + 회사 카드 수집(회사당 1회 XP +10).
  3. 섹터 세트(예: 가상 반도체 6/6)·국가 세트(US/KR/JP 가상 유니버스)·시즌 한정 세트 완성 시 배지·XP·코스메틱 지급.
  4. 세트 완성은 P&L 독립 축하 연출(컨페티·badge-earned haptic) 허용.
- **대안 흐름:** 읽기만 한 회사는 발견(progress)만, 첫 거래 전 unlock 미완료; 상장폐지/이벤트 종료 회사는 시즌 아카이브에 표시; 대회 계좌 첫 거래는 contest source로 태깅하되 개인 도감 중복 파밍 불가.
- **예외 흐름:** 첫 거래 이벤트 중복은 idempotency로 1회 지급; 회사 ID 변경/마이그레이션 시 legacy alias 매핑; 미성년/고위험 상품 제한 회사는 읽기 전용 발견만 허용.
- **사후조건:** collection_entry_unlocked·collection_set_completed 이벤트 기록; XP 원장에 source=collection 태그; 공유카드는 도감/배지 중심이며 수익률 노출 없음.
- **관련 정책:** GRF 진행 보상, LEG-07 P&L 독립 축하, LEG-16 가상 발행사, tracking-plan.md collection events, game-modes.md §2.4.

### UC-B37 · 푸시 권한 프라이밍·알림함 fallback·윈백

- **액터:** 복귀 사용자, 휴면 사용자, 시스템(앱), OS 푸시 서비스(APNs/FCM)
- **목표:** 첫 가치 순간 이후에만 알림 권한을 요청하고, 거부 사용자는 인앱 알림함으로 동일 정보를 받으며, 휴면 사용자를 조용시간·일일상한을 지켜 윈백한다.
- **사전조건:** 사용자가 최소 1개 가치 순간(입성 배지, 도감 첫 unlock, 시나리오 완주, 첫 리그 참가 중 하나)을 경험; consent tier와 OS permission 상태 확인 가능.
- **주요 흐름:**
  1. 첫 가치 순간 이후 인앱 soft ask 표시("속보·도감·리그 마감만 알려드릴게요") → 사용자가 계속을 탭하면 OS permission prompt.
  2. granted면 notification channel preference 기본값(속보/도감/리그/시스템)과 quiet hours(22-08) 안내.
  3. denied면 OS 재요청을 반복하지 않고 인앱 알림함 fallback을 활성화. 홈 상단/알림 아이콘에 읽지 않은 항목만 표시.
  4. 발송 후보는 D1/D3/D7/D14/D30 정책표, 조용시간, 손실 포지션 억제, 일일 1-2건 상한, 최소 간격 20분을 통과해야 sent 상태가 된다.
  5. 휴면 사용자(최근 14일 이상 핵심 게임 액션 없음)는 D14/D30 win-back만 대상. 메시지는 "새 시즌/도감/시나리오" 중심이며 손익·손실회복·매수 CTA 금지.
- **D1/D3/D7/D14/D30 정책표:**

| 코호트 시점 | 허용 채널 | 예시 주제 | 상한/가드 |
|---|---|---|---|
| D1 | OS push 또는 inbox | 입성 배지 후 다음 미션, 첫 도감 세트 힌트 | 1건, 가치 순간 없으면 발송 금지 |
| D3 | OS push 또는 inbox | 좋은 판단 스트릭, 신규 가상회사 탐험 | 1건, 거래 CTA 금지 |
| D7 | OS push 또는 inbox | 주간 리그/시나리오 마감, 세트 완성 임박 | 1-2건/day 전체 상한 내 |
| D14 | win-back | 새 시즌 도감·시나리오 개방 | 휴면 사용자만, 조용시간 준수 |
| D30 | final win-back/inbox | 시즌 리셋·새 컬렉션 요약 | 1건, 이후 30일 snooze |

- **대안 흐름:** permission already granted이면 soft ask 생략; provisional/ephemeral 권한(iOS)은 quiet delivery로 시작; 사용자가 채널을 끄면 해당 topic은 inbox-only; B2B/미성년 계정은 교사/보호자 정책 우선.
- **예외 흐름:** 조용시간 후보는 `notification_suppressed_quiet_hours` 기록 후 다음 다이제스트로 이동; 일일 상한 초과는 suppressed_cap; 손실 포지션 악재는 delayed_inbox; OS token invalid는 inbox-only로 폴백.
- **사후조건:** permission_prompted, permission_granted/denied, notification_sent/opened, notification_suppressed_quiet_hours/cap 이벤트 기록; 사용자는 OS 권한 거부 상태에서도 인앱 정보 접근 가능.
- **관련 정책:** NWS-05/06, LEG-07, GRF-15, DP-10, tracking-plan.md notification events.
...
| UC-B01/A01 | 게스트 온보딩 첫 매수 | **P0** | 30초 활성화·day-one 정체성 배지(입성 / 플레이어 탄생). 첫인상. |
...
| UC-A15 | 파생·마진 종목 상세 화면 | **P2** | 고위험 상품 상태·마진·고지 표면. LEV/ACC 스키마 선행. |
| UC-A16 | 레버리지 설정 플로우 | **P2** | 레버리지 juice와 리스크 고지의 핵심. MARGIN 계좌 분기 선행. |
| UC-A17 | 청산가·마진콜 경고 | **P2** | 청산 리스크 투명성·wellbeing 알림. LEV-04/05·NWS-05 선행. |
| UC-A18 | 부분청산 통지 | **P2** | 강제청산 사유·복기 표면. 서버 권위 청산 엔진 선행. |
| UC-A19 | 옵션 그릭스 표시 | **P3** | 옵션 IP2 표면. MKT-15/ORD-24/ACC-20 선행. |
| UC-A20 | 펀딩레이트 카운트다운 | **P3** | 무기한/IP2+코인 표면. MKT-16/ACC-18 선행. |
...
| UC-B36 | 도감/컬렉션 발견·세트 완성 | **P1** | 수집 pillar 정본. 첫 거래 신규 가상회사 XP와 세트 보상. |
| UC-B37 | 푸시 권한·알림함·윈백 | **P1** | 리텐션 루프. soft ask→OS prompt→inbox fallback, quiet-hours 준수. |
...
| UC-A21~A27 | deferred fun-surface UCs | **P3** | 차트·미시구조·뉴스·펄스·이벤트·유상청약 상세. 엔진/정책 안정 후 상세화. |
...
- **R4:** 매매 체결·실현/미실현 P&L 축하 연출(컨페티·팡파레) 및 과잉거래 유도 푸시 금지. 축하는 P&L 독립 성취(입성 / 플레이어 탄생, 실력지표, 좋은 판단 스트릭, 배지/레벨, 도감/세트)에만 허용.
