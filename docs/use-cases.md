...
**한줄요약:** 게스트 온보딩·핵심 트레이딩·포트폴리오 회계부터 학습·게임화·소셜·수익화·법무/개인정보까지 42개 유즈케이스를 표준 형식(액터·목표·사전조건·흐름·사후조건·관련정책)으로 정리하고, 액터 정의·유즈케이스 다이어그램·우선순위 표를 제공한다.
...
| **학습자** | 학습 콘텐츠(개념카드·스킬트리·용어사전·시나리오)를 소비하는 모든 사용자. | UC-B04~B07 |
| **복귀 사용자** | 데일리 미션·스트릭을 이어가는 리텐션 계층. | UC-B08, UC-B09 |
| **경쟁 사용자(가입 필요)** | 리그·리더보드에 참여하는 가입 사용자. | UC-B12, UC-B16 |
...
| UC-B05 | 학습 트랙(스킬트리) 노드 수강·퀴즈 | B | 학습자 |
...
- **주요 흐름:**
  ...
  2. 연령 게이트: 생년월일 입력(만14세 한/16세 EU/13세 미 임계 분기, UC-B02 위임).
  3. 온보딩 1회 전면 면책 동의(체크박스) → 동의 타임스탬프·문구 버전 기록.
...
- **사후조건:** 유효한 게스트 계좌 1개; 'TUTORIAL' 태그 거래 1건; 면책·연령 동의 이력 저장; day-one 배지 1개 획득.
- **관련 정책:** 연령게이트(PIPA14/GDPR16/COPPA13, LEG-05), 온보딩 1회 면책+동의 로그(LEG-02), 매매 축하 연출 금지(R4/LEG-07), 데이터 최소화·지연 가입(DP-01), schemaVersion 마이그레이션(ACC-09/DP-04), 튜토리얼 거래 격리(ACC-10).
...
- **사전조건:** 주문유형 해금(리얼모드/고급주문); 대기주문 엔진 매 틱 평가 가동.
- **주요 흐름:**
...
- **관련 정책:** 대기주문 엔진(ORD-08, 도메인 순수 TS), 결정론 시세 재현·리플레이(MKT-01), TIF·틱사이즈(ORD-12/ORD-02), '지정가란?' 학습 연계(UC-B05).
...
- **대안 흐름:**
  - A1(스탑리밋): stop 발동 시 limit 주문 생성 → limit 규칙 체결.
  - A2(트레일링스탑): 고점 대비 trailPct 하락 시 발동 → 시장가 전환.
  - A3(OCO): 손절+익절 한 그룹, 하나 체결 시 나머지 자동 취소.
- **예외 흐름:**
...
- **관련 정책:** 손절 미션·'손절 준수율' 배지 회계 근거(ACC-11), 리스크관리 성취에만 축하 허용(R4 예외/LEG-07), 레버리지 원금초과 고지(ACC-15), STOP/TP는 쉬운모드도 노출(ORD-10).
...
- **사전조건:** 개인 연습 계좌 활성; 리얼모드 해금(또는 해금 가능).
- **주요 흐름:**
...
- **주요 흐름:**
  ...
  2. 연령 확인(생년월일) 진입(UC-B02 분기).
  3. 온보딩 1회 전면 면책 동의(타임스탬프·문구 버전 기록).
...
- **사후조건:** 게스트 계정 로컬 생성; TUTORIAL 거래 1건 별도 태그; 첫 배지 보유; 면책 동의 이력(버전·타임스탬프) 저장.
- **관련 정책:** R4 매매 축하 금지(LEG-07), 온보딩 1회 전면 고지(LEG-02), 연령 게이트(LEG-05), day-one achievement 보장, 튜토리얼 거래 별도 태그(ACC-10).
...
- **액터:** 신규 사용자(게스트)
- **목표:** 생년월일 기반 관할별 아동 임계연령을 판별해 미달자는 진행 차단, 통과자만 온보딩 계속.
- **사전조건:** 온보딩 진행 중; 디바이스 로케일/관할 추정 가능.
- **주요 흐름:**
  1. 연령 확인 화면에서 생년월일(최소 필드)·수집 사유 표시.
  2. 사용자 생년월일 입력.
...
- **관련 정책:** PIPA 만14세·GDPR 16세·COPPA 13세(LEG-05/DP-13), 데이터 최소화(생년월일만/DP-01), 청소년 시장은 B2B 트랙 별도(MON-11).
...
- **대안 흐름:**
  ...
  - 5a(마이그레이션 충돌-동일 이메일 계정 존재): '기존 계정 로그인' 또는 '새 계정 병합' 선택, 병합 시 append-only 원장으로 거래 이력 합침.
...
- **사후조건:** 서버 계정 생성·로컬 이관 완료; 멀티 디바이스 동기화 가능; 게스트 데이터 유실 없음.
- **관련 정책:** 게스트 우선·지연 가입(DP-01/ACC-06), 이벤트소싱 append-only 원장(ORD-21), 서버 UTC 시각 권위(GRF-04), 데이터 최소화(DP-01), 계좌 병합 규칙(ACC-06/DP-11).
...
### UC-B05 · 학습 트랙(스킬트리) 노드 수강 및 퀴즈 완료

- **액터:** 학습자(모든 사용자)
- **목표:** T1 기초→T2 실전→T3 심화 스킬트리에서 노드(개념카드+미션+퀴즈)를 순차 완료해 XP·배지 획득·다음 노드 언락.
- **사전조건:** 학습 탭 노출(첫 미션 완료 후); 이전 노드 완료(선형 언락); LearningContext 초기화.
- **주요 흐름:**
  1. 학습 탭에서 활성 다음 노드 선택.
  2. 개념 카드 1~3장 스와이프 학습.
  3. 인터랙티브 미션(예: 3종목 분산 매수)을 앱 내 실제 매매로 수행.
  4. 미션 완료를 transactions/holdings 상태 구독으로 자동 판정.
  5. 미니 퀴즈로 이해도 확인.
  6. XP + 노드 배지 + (선택) 소액 가상현금 크레딧 지급.
  7. 다음 노드 언락, 홈 스트릭 +1.
- **대안 흐름:**
  - 3a(조건 미충족 이탈): 노드 '진행 중' 유지, 나중에 충족 시 자동 완료.
  - 5a(퀴즈 반복 오답): 관련 개념 카드로 유도, 정답률 콘텐츠 KPI 기록.
  - 6a(크레딧 상한): 일일 상한·쿨다운 적용, 초과분 배지·코스메틱 대체.
  - 6b(크레딧 회계): cash 직접 가산 금지, externalFlows 'MISSION' 기록(분모 오염 방지).
  - 2a(원격 버전 갱신): 완료 노드 상태 보존, 신규 노드만 추가.
  - 1a(T3 잠금): T2 60% + 리얼모드 해금 미충족 시 잠금·조건 안내.
- **사후조건:** 노드 완료·XP·배지 저장(ts.learning.v1); 미션 크레딧 externalFlows 기록; 다음 노드 언락.
- **관련 정책:** 보상은 학습·좋은 습관에(매매 자체엔 XP 0/LEG-07), 크레딧 상한·쿨다운(ACC-11/MON-04), 외부현금흐름 분리 회계(ACC-03/ACC-07), LearningContext 분리(ACC-10).
...
- **액터:** 복귀 사용자(모든 사용자)
- **목표:** 홈에 노출된 오늘의 미션 2~3개(학습·포트폴리오 점검·지정가 등)를 완료해 XP를 얻고 스트릭을 유지.
...
- **관련 정책:** 데일리 미션 2~3개 상한(LEG-07/GRF-15), 알림 1일 1~2회 상한(LEG-07), 스트릭은 학습에만(GRF-15), 보상은 좋은 습관에(LEG-07).
...
- **액터:** 복귀 사용자(모든 사용자)
- **목표:** 매일 로그인+학습 미션 완료로 스트릭을 이어가고, 미접속 시 프리즈로 단절을 완충한다.
- **사전조건:** 스트릭 카운터 초기화; 서버 UTC 일자(v1) 또는 로컬 임시(P0); 무료 프리즈 주 1회 지급.
- **주요 흐름:**
  1. 당일 로그인 + 데일리 미션 1개 이상 완료.
...
- **관련 정책:** 스트릭=매일 학습(매일 거래 아님/GRF-15), 프리즈 필수 완충, 서버 UTC 시각 권위(GRF-04/DP-06), 현금성 스트릭 보상 금지(LEG-04).
...
- **주요 흐름:**
  1. 프리미엄 게이트 기능(광고제거·고급분석·리얼모드·다계정·대규모 대회개설) 탭.
...
- **대안 흐름:**
  - 1a(상한 소진): 일일 상한(시작자산 5%=$5,000) 소진 시 가상현금 차단, 대체 리워드(코스메틱·백테스트권·프리미엄 1일)만.
...
- **관련 정책:** 리워드 비디오 주력(MON-03), 가상현금 리필 상한·쿨다운·하루 3회(MON-04/ACC-11), 매매 직후 전면광고 금지(MON-03/LEG-07), 무료 리셋 병행(ACC-02), 외부현금흐름 분리(GRF-03/ACC-03), ATT/광고동의(DP-10/MON-03), SSV 서버 검증(MON-08).
...
- **대안 흐름:**
  ...
  - 1a(광고 리필 대안): 리워드 광고 리필(UC-B25) 선택 가능하나 무료 리셋도 항상 제공.
...
- **관련 정책:** 파산 시에만 리셋+쿨다운(ACC-02/ACC-16), 무료 리셋 병행(도박 넛지 회피/MON-04), 영속 정체성 보존(ACC-02), 시즌 스냅샷 보존(ACC-13), 파산 히스테리시스(ACC-02), 리셋 시 리더보드 재배치(GRF-01/ACC-13).
...
- **액터:** 사용자(모든 사용자)
- **목표:** 온보딩 1회 전면 면책·약관·개인정보 동의를 기록하고, 이후 설정에서 상시 열람·관리.
- **사전조건:** ToS·개인정보처리방침 문서 게시; 동의 버전 관리 체계.
- **주요 흐름:**
  1. 온보딩에서 면책·약관·개인정보 1회 전면 고지·동의(버전·타임스탬프 기록).
...
- **관련 정책:** 면책 3단계(온보딩 전면·상시 배너·데이터 지연/LEG-02), 동의 분리·묶음 금지(DP-10/LEG-05), 동의 이력 관리(LEG-02/DP-02), 데이터 출처·지연 고지(MKT-05).
...
| UC-B05 | 학습 스킬트리·퀴즈 | **P1** | 학습 코어·보상 루프. LearningContext. |
...
| GRF-01~15 | 게임화·랭킹 공정성 | [gamification-fairness-policy](./gamification-fairness-policy.md) |
