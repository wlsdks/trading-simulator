...
> **[성격 고지]** 본 문서는 제품 계측(instrumentation)의 **단일 진실원본(single source of truth)** 이다. PRD §4의 North Star(WAP·주간 활성 플레이어)·활성화 퍼널·K-factor·안티메트릭(§4.2, GRF-15), §16의 "각 스텝 독립 계측·A/B로 이탈 스텝 제거" 주장은 **모두 본 문서에 정의된 이벤트 위에서만 관측·검증 가능**하다. 이벤트가 여기에 없으면 그 지표는 존재하지 않는다.
>
> **제품 성격:** 이 제품은 **재미로 하는 주식 게임(fun-first, 가짜 돈)** 이다. 계측은 참여·리텐션·수집·대회 등 **재미 지표** 중심이며, 교육·학습성과(pre/post·mastery·SRS·retention·transfer) 계측은 다루지 않는다.
>
...
현행 상태: 애널리틱스 SDK 0, 이벤트 스키마 0, 어트리뷰션 0, 실험 프레임워크 0. 즉 **PRD가 선언한 모든 성공지표가 측정 불가**다. Phase 0 회계 하드닝이 "게임화·소셜의 하드 선행 과제"이듯, **경량 텔레메트리 경로는 모든 KPI·A/B·규제증거의 하드 선행 과제**다. 계측 없이 소프트런치하면 첫 코호트(가장 값비싼 초기 행동 데이터)가 영구 소실된다.
...
- **이벤트명:** `snake_case`, **`객체_동작(과거형)`** 패턴. 예: `order_submitted`, `badge_earned`, `paywall_viewed`. 문장/공백/대문자 금지.
- **속성명:** `snake_case`. 불리언은 `is_`/`has_` 접두. 열거형은 소문자 문자열(`account_mode: "easy"|"real"`).
- **도메인 접두 태그(속성 `category`):** 이벤트를 7도메인으로 분류 — `onboarding | trading | gamification | social | monetization | integrity | system`. policies.md 도메인 코드와 1:1 정렬.
...
| `activation_badge_earned` | '입성 / 플레이어 탄생' 배지 지급 | `badge_id` | T0 |
...
### 5.3 게임화·수집 (category=gamification) — **NSM 핵심**
| 이벤트 | 트리거 | 핵심 속성 | min_tier |
|---|---|---|---|
| `risk_stop_set` | 손절/익절 **설정(플레이 액션)** | `order_type`, `is_first_ever` | T1 |
| `streak_extended` / `streak_freeze_used` | 스트릭 유지/프리즈 | `streak_len`, `freeze_source`(free/reward, **paid 금지**) | T1 |
| `badge_earned` | 배지 지급 | `badge_id`, `badge_type`(collection/risk/exploration/journey) | T1 |
| `level_up` | 레벨업 | `new_level` | T1 |
| `xp_earned` | XP 지급 | `source`(risk_adjusted/scenario/new_company/stop_adherence/skill_play/collection), `xp_amount`, `source_id` | T1 |
| `skill_play_completed` | positive-EV skill play 판정 | `skill_type`(earnings_preposition/overshoot_fade/deny_rumor_distrust), `xp_amount` | T1 |
| `collection_entry_unlocked` | 가상회사 도감 collected | `company_id_hash`, `sector`, `country`, `is_first_trade` | T1 |
| `collection_set_completed` | 도감 세트 완성 | `set_type`(sector/country/season/event_arc), `set_id`, `xp_amount` | T1 |

> XP 정합: `order_filled`·거래량·거래대금·P&L은 XP source가 아니다. `xp_earned.source=trading_volume`은 스키마 lint에서 거부한다.

### 5.3.1 알림/푸시 (category=gamification/system)
| 이벤트 | 트리거 | 핵심 속성 | min_tier |
|---|---|---|---|
| `permission_prompted` | 인앱 soft ask 또는 OS prompt 표시 | `prompt_type`(soft/os), `trigger`(first_value_moment/settings) | T1 |
| `permission_granted` / `permission_denied` | OS 권한 결과 | `channel`(push), `source`(os/settings) | T1 |
| `notification_inbox_item_created` | 인앱 알림함 항목 생성 | `topic`(news/collection/league/system/winback), `reason` | T1 |
| `notification_sent` | OS push 발송 | `topic`, `cohort_day`(d1/d3/d7/d14/d30), `was_dormant` | T1 |
| `notification_opened` | push 또는 inbox 항목 열람 | `topic`, `surface`(push/inbox) | T1 |
| `notification_suppressed_quiet_hours` | 조용시간 억제 | `topic`, `next_delivery`(digest/inbox) | T1 |
| `notification_suppressed_cap` | 1~2/day 상한 억제 | `topic`, `daily_count` | T1 |
...
| PRD 지표 | 정의(이벤트) | 계측 상태 |
|---|---|---|
| **NSM: WAP** | 주간 distinct user with (`streak_extended` OR `badge_earned` OR `league_promoted` OR `group_created` OR `diversification_milestone_reached`) ≥1 | §7.1 쿼리 |
...
### 7.1 North Star — WAP(주간 활성 플레이어)
```sql
-- WAP: 주당 참여·수집·대회 행동 1회 이상 수행한 distinct 유저
SELECT date_trunc('week', ts_server) AS wk,
       count(DISTINCT user_id)       AS wap
FROM   events
WHERE  event_name IN (
         'streak_extended',
         'badge_earned',
...
- **(구) [삽입 블록 A] — PRD.md 수정 지시 / (구) [삽입 블록 B] — policies.md INST 도메인**: 타 문서(PRD·policies)의 편집을 지시하던 메타블록이었다. 본 문서는 계측 SSOT 역할만 유지하기 위해 폐기했다. 계측 거버넌스(INST-01~05)는 policies.md에서, 스택·오픈이슈 반영은 PRD.md에서 각각 직접 관리한다.
